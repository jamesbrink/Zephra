...
# The roomy volume the big release sources and the mirror live on. A release is tens of
# gigabytes and does not belong on a boot volume, so every source and output below that is not
# the app's own folder is defined off this one variable; set it on any other Mac.
EXTERNAL_MODELS ?= /Volumes/ExternalStorage/Models
...
# to EXTERNAL_MODELS. `make quantize-ltx2` is the same build by hand;
...
LTX2_MODELS  ?= $(EXTERNAL_MODELS)/ZephraModels
...
# `aws s3 sync "$(MIRROR_DIR)" s3://bucket/ --delete` mirrors it. The default is EXTERNAL_MODELS:
# the variants together are tens of gigabytes, which does not belong on a boot volume.
...
MIRROR_DIR          ?= $(EXTERNAL_MODELS)/ZephraMirror
...
MLX_PACKAGES := ZephraMLXKit:ZephraMLXKit-Package ZephraUpscaleRealESRGAN:ZephraUpscaleRealESRGAN \
                Flux2Kit:Flux2Kit \
