...
///
/// The filter bar, in `LibraryPaneHeader`, shows only when it has something to say — a
/// narrowed query or a live selection — so an untouched library shows the grid alone.
///
...
struct LibraryPane: View {
    ...
    var body: some View {
        content
            .safeAreaInset(edge: .top, spacing: 0) { LibraryPaneHeader(selection: selection) }
