...
/// detached task, and the `NSImage` is made on the main actor around the `CGImage` that comes
/// back, which is immutable and so may cross.
@MainActor
@Observable
final class ImageCache {
    ...
    /// flash of nothing.
    func cached(_ image: GeneratedImage, _ kind: Kind) -> NSImage? {
    ...
    func load(_ image: GeneratedImage, _ kind: Kind) async -> NSImage? {
        ...
        let made = cached(image, kind) ?? NSImage(cgImage: decoded, size: .zero)
    ...
    func referenceThumbnail(_ png: Data) async -> NSImage? {
        ...
        let made = references.object(forKey: cacheKey) ?? NSImage(cgImage: decoded, size: .zero)
    ...
    private func store(for kind: Kind) -> NSCache<NSUUID, NSImage> {
        switch kind {
        case .full: fullSize
