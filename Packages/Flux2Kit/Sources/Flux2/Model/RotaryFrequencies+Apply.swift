...
extension RotaryFrequencies {
    ...
    /// same arithmetic as the reference's real-and-imaginary path written out in real terms.
    ///
    /// The pairing is **adjacent** channels, `(0,1), (2,3), ...`, not two halves of the head.
    /// Both conventions exist in the wild and they are not interchangeable. The text encoder in
    /// this same package uses the other one.
    ///
    /// The multiply runs in float32 whatever `x` is, as the reference does: at bfloat16 the
    /// angles for the far positions of a 4096-token image lose too much.
    public func rotate(_ x: MLXArray) -> MLXArray {
        let shape = x.shape
        let (batch, positions, heads, headDim) = (shape[0], shape[1], shape[2], shape[3])
        let pairs = headDim / 2

        let split = x.asType(.float32).reshaped([batch, positions, heads, pairs, 2])
        ...
        let cosine = cos.reshaped([1, positions, 1, pairs])
        let sine = sin.reshaped([1, positions, 1, pairs])
