...
extension Flux2PixelBuffer {
    /// Decodes a picture's bytes into `[1, 3, height, width]` in the range -1 to 1, scaled to
    /// cover at most a megapixel and cropped so each edge is a multiple of sixteen.
    ///
    ...
    public static func pixels(from data: Data) throws -> MLXArray {
        ...
        let target = Flux2ImageFitting.target(width: image.width, height: image.height)
