...
/// its shape, and then trims each edge to a multiple of sixteen so the latent grid comes out
/// whole. The shape is kept on purpose: a reference squashed to the output's aspect would be
...
public enum Flux2ImageFitting {
    ...
    /// The multiple every edge is trimmed to: the autoencoder's eightfold reduction times the
    /// two-by-two packing.
    public static let alignment = 16

    /// The size a `width` by `height` picture is fitted to.
    public static func target(width: Int, height: Int) -> Target {
