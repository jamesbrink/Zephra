...
extension WanBackend {
    ...
    nonisolated(nonsending) public func availability(
        ...
        case .localDirectory:
            ...
            guard candidates.allSatisfy({ LocalSnapshot.wan.missingEntry(in: $0) != nil })
            else { return .available }
            let missing = LocalSnapshot.wan.missingEntry(in: candidates[0]) ?? "its weights"
            return .missing(reason: "Not built yet: \(missing) is missing. Run `make quantize-wan`.")
        case .huggingFace:
            if LocalSnapshot.wan.packedVariant(of: descriptor, in: locations) != nil {
                return .available
            }
            let release = LocalSnapshot.wanRelease.downloadedRelease(of: descriptor, in: locations)
