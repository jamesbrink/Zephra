...
nonisolated enum ReferenceImageDecoding {
    ...
    ///
    /// A copy of the file every backend package keeps, because no backend package may import
    /// another. Here the picture becomes the clip's first frame rather than something to start
    /// from; scaling it to the clip's size is the pipeline's job.
