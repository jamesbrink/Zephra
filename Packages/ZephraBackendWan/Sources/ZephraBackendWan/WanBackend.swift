...
/// Runs Wan 2.2 models through Zephra's own MLX pipeline.
///
/// The instance is deliberately not Sendable: it owns an `WanPipeline`, which holds MLX
...
/// methods.
public nonisolated final class WanBackend: ImageGenerationBackend {
    /// The model family this backend serves.
    public static let backendID = BackendID.wan
    ...
    /// The VAE tile the engine set for the run about to start; see `WanBackendFactory`. Read
    /// as each run's request is built and scaled to this family's cell.
    ...
    /// streamed, the 30 transformer blocks and the 24 encoder layers are read from disk on every pass.
    nonisolated(nonsending) public func load(
        ...
        let streaming = residency == .streamed ? WanStreaming(depth: environment.streamDepth) : nil
        do {
            try pipeline.loadModel(
                at: localPath,
                activation: WanActivationPrecision.resolve(environment: environment),
                streaming: streaming
            ) { progress in
                onProgress(WanProgressMapper.event(from: progress))
            }
    ...
    nonisolated(nonsending) public func generate(
        ...
        let onPreview: WanPipeline.PreviewHandler? = PreviewFrameReporter.handler(
            interval: environment.previewInterval, onProgress: onProgress)
        let clip: WanClip
        do {
            var request = try WanRequestMapper.request(for: settings, descriptor: descriptor)
            request.vaeTile = WanRequestMapper.vaeTile(from: tile.value)
            clip = try pipeline.generate(
                request,
                onProgress: { progress in onProgress(WanProgressMapper.event(from: progress)) },
                onPreview: onPreview)
        } catch let error as CancellationError {
            throw error
        } catch {
