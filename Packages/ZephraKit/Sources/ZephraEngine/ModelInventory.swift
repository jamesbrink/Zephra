...
public final class ModelInventory {
    ...
    @ObservationIgnored private let builtIn: URL
    @ObservationIgnored private let remove: @Sendable (ModelStorageItem) throws -> Void
    @ObservationIgnored private var generation = 0

    /// An inventory over `catalog`, reading `cache` and `builtIn`. The defaults are the real
    /// places; a test passes a scratch directory and a `remove` that does not use the Trash.
    public init(
        catalog: [ModelDescriptor] = ModelCatalog.all,
        cache: URL = HubCache.directory(),
        builtIn: URL = ModelCatalog.localModelsDirectory,
    ...
    /// Where downloads land, for the window to show and open.
    public var downloadsDirectory: URL { cache }
    /// Where variants packed on this Mac land.
    public var builtDirectory: URL { builtIn }
    ...
    public func refresh() async {
        generation += 1
        let mine = generation
        let found = ModelStorage.items(for: catalog, cache: cache, builtIn: builtIn)
        items = found
