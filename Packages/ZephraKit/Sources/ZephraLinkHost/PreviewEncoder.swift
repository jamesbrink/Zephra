...
public enum PreviewEncoder {
    ...
    public nonisolated static func encode(_ preview: GenerationPreview) -> Data? {
        guard preview.isWellFormed, let image = makeImage(preview) else { return nil }
    ...
    /// The pixels as a `CGImage`. Opaque, so the alpha byte the engine leaves at 255 is skipped
    /// rather than being premultiplied into three channels that are already final.
    private nonisolated static func makeImage(_ preview: GenerationPreview) -> CGImage? {
