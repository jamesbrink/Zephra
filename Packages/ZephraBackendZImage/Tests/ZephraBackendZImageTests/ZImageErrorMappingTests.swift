...
struct ZImageErrorMappingTests {
    ...
    func brokenTransfersAreDownloadFailures() {
        ...
        #expect(offline == .downloadFailed(ZImageErrorMapping.message(ModelResolutionError.networkUnavailable)))
        ...
        #expect(
            ZImageErrorMapping.downloadError(broken, descriptor: descriptor)
                == .downloadFailed(ZImageErrorMapping.message(broken))
        )
        #expect(ZImageErrorMapping.message(broken).contains("mzbac/Z-Image-Turbo-8bit"))
    }
    ...
    func localizedErrorsKeepTheirText() {
        ...
        #expect(ZImageErrorMapping.message(error) == "Model not found: mzbac/nope")
    }
    ...
    func silentErrorsFallBackToTheCaseName() {
        ...
        #expect(ZImageErrorMapping.message(ZImagePipeline.PipelineError.modelNotLoaded) == "modelNotLoaded")
        #expect(
            ZImageErrorMapping.message(ZImagePipeline.PipelineError.weightsMissing("dit.safetensors"))
    ...
    func cancellationHasAMessage() {
        ...
        #expect(!ZImageErrorMapping.message(CancellationError()).isEmpty)
    }
}
