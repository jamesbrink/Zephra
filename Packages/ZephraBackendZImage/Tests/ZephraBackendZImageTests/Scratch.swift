...
///
/// Every suite here that touches the file system builds its fixture through this, so a failing
/// test leaves nothing behind and two tests never share a path.
final class Scratch {
    /// The root every path is created under.
    let root: URL

    init(_ label: String = "Zephra") {
    ...
    @discardableResult
    func link(_ path: String, to destination: String) throws -> URL {
        let url = root.appending(path: path)
        let files = FileManager.default
        try files.createDirectory(
