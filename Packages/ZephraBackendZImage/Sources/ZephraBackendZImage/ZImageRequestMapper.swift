...
nonisolated enum ZImageRequestMapper {
    ...
    static func request(
        ...
        if let reference = clamped.reference {
            request.referenceImage = try ReferenceImageDecoding.cgImage(at: reference.url)
            request.referenceStrength = Float(reference.strength)
        }
        return request
    }
