...
extension QwenImage21Backend {
    ...
    nonisolated(nonsending) public func availability(
        ...
        case .localDirectory:
            ...
            guard candidates.allSatisfy({ LocalSnapshot.qwenImage21.missingEntry(in: $0) != nil })
            else { return .available }
            let missing = LocalSnapshot.qwenImage21.missingEntry(in: candidates[0]) ?? "its weights"
            return .missing(
                reason: "Not built yet: \(missing) is missing. Run `make quantize-qwen21`.")
        case .huggingFace:
            if LocalSnapshot.qwenImage21.packedVariant(of: descriptor, in: locations) != nil {
                return .available
            }
            let release = LocalSnapshot.qwenImage21Release.downloadedRelease(
