...
extension LTX2Backend {
    /// Resolves the pack, downloading it if it is not on this Mac, and returns the directory it
    /// is in. This is the pack, not what gets loaded; `build` is next.
    ///
    /// Three places are looked at before anything is fetched: the packed variant, which makes
    /// the pack unnecessary and may even have been deleted; the folder the user keeps models
    /// in; and the hub cache in either layout, which is read as a fallback and never written,
    /// so a Mac seeded by `make prefetch-ltx2` skips the sixty-nine gigabytes.
    nonisolated(nonsending) public func ensureAvailable(
        ...
        case .huggingFace:
            ...
            // from; nil means the mirror has not got it, and the pack comes next.
    ...
    nonisolated(nonsending) public func build(
        ...
        do {
            return try LTX2SnapshotBuild.pack(
                release: localPath,
