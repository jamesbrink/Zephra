...
enum WanResidentParameters {
    /// The module path of the text encoder's blocks.
    static let textEncoderLayers = UMT5TextEncoder.blocksKeyPrefix
    /// The module path of the transformer's blocks.
    static let transformerBlocks = "blocks"

    /// Reads every parameter into memory, except the streamed stacks when `streamed`.
    static func eval(_ loaded: WanPipeline.Loaded, streamed: Bool) {
        let always = loaded.autoencoder.parameters().flattenedValues()
        guard streamed else {
            MLX.eval(
